...
def extract_h3d(feats):
    ...
    root_data, ric_data, rot_data, local_vel, feet_l, feet_r = unpack(feats, [[4], [63], [126], [66], [2], [2]], "i *")
...
@hydra.main(config_path="../configs", config_name="compute_body_part_guoh3dfeats", version_base="1.3")
def compute_body_part_guoh3dfeats(cfg: DictConfig):
    base_folder = cfg.base_folder
    output_folder = cfg.output_folder
    force_redo = cfg.force_redo
    part_name = cfg.part_name

    assert part_name in ["head", "left_arm", "right_arm", "torso", "left_leg", "right_leg"]

    from src.guofeats import joints_to_guofeats
    from src.guofeats.motion_representation import rearrange_guofeats
    ...
    iterator = loop_amass(base_folder, output_folder, ext=".npy", newext=".npy", force_redo=force_redo)
    ...
    for motion_path, new_motion_path in iterator:
        ...
        try:
            features = rearrange_guofeats(joints_to_guofeats(joints))[part_name]
            features_m = rearrange_guofeats(joints_to_guofeats(joints_m))[part_name]
